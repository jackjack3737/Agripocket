...
import { queryKnowledgeBasePrioritized } from "../web/server/kbQuery.mjs";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!profilo?.localita?.trim()) {
      res.status(400).json({ error: "Imposta la localitÃ  nel profilo per il calcolo irrigazione." });
      return;
    }

import { calculatePolygonAreaSqm } from "./polygonArea";
...
/** Livello sole/ombra per poligoni disegnati in mappa. */
...
/** Poligoni esposizione (include mappe vecchie tipo `ombra`). */
...
/** Uscita centralina (1–8); null se non impostata (compatibilità mappe vecchie). */
...
/** Anelli validi del contorno prato (supporto multi-poligono: davanti + retro casa). */
...
/** Superficie totale m² (somma di tutti i poligoni base). */
...
/** Accetta oggetto o JSON string (alcuni client Supabase). */
...
function normalizeZone(z) {
  ...
  if (z.tipo === "ombra") {
    z = { ...z, tipo: "esposizione", livello: "ombra" };
  }
...
export function anonymizePratoZoneForStorage(raw) {
  ...
  z.poligoni = (z.poligoni || []).map((ring) =>
    ring.map((p) => ({ lat: roundCoord(p.lat), lng: roundCoord(p.lng) })),
  );
...
/** % equivalente ombra (pesata: ombra 100%, mezz'ombra 50%, sole 0%). */
...
/** Stima fascia ombra profilo (da aree pesate in mappa). */
...
/** Aree ombra/mezz'ombra per semina (esclude zone «sole»). */

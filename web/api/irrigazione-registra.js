...
import {
  ...
  oggiIso,
} from "../server/irrigazioneUtente.mjs";
import { normalizzaInputIrrigazione } from "../server/irrigazioneInput.mjs";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!profilo?.localita?.trim()) {
      res.status(400).json({ error: "Imposta la località nel profilo." });
      return;
    }
